...
const estudiarTodo = new TodoModel(
  1,
  "estudiar para el parcial",
  "debo estudiar para el parcial de logica y para el parcial de php objetos",
  false,
  "22/22/2025"
);

const estudiarCompleted = new TodoModel(
  2,
  "estudiar para el parcial 2",
  "debo estudiar para el parcial de logica y para el parcial de php objetos",
  true,
  "22/22/2025"
);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (storedTodos) {
    allTodoList = JSON.parse(storedTodos).map(
      (todo) =>
        new TodoModel(
          todo.id,
          todo.title,
    ...
    allTodoList = [
      ...
      new TodoModel(
        2,
        "Estudiar CSS",
        "Estudiar para el examen de CSS",
        false,
        "2025-05-02"
      ),
    ];
...
function buildTodos(todo) {
  ...
  todoContainer.className =
    "todo-container d-flex flex-column flex-md-row justify-content-between p-4 align-items-center border border-3 border-secondary-dark rounded-4 gap-4 mt-3 ";
...
function buildTodoCompleteds(todo) {
  let todoContainer = document.createElement("div");
  todoContainer.className =
    "todo-completed-container d-flex flex-column flex-md-row justify-content-between p-4 align-items-center border border-3 border-secondary-dark rounded-4 mt-3 gap-4";
  todoContainer.addEventListener("click", function (e) {
    e.stopPropagation();
    e.preventDefault();
    constructAndShowTodoModal(todo);
...
function buildTodoCheckAndTitle(todo) {
  ...
  checkAndTitleContainer.className =
    "check-and-title-container d-flex gap-2 align-items-center";
  ...
  const strokeCircle = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "circle"
  );
  ...
  const checkLine = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "polyline"
  );
...
function buildDateIconAndDateText(todo) {
  ...
  dateIconAndDateTextContainer.className =
    "dateIcon-and-dateText-Container d-flex align-items-center gap-2 justify-content-between position-relative";
...
function toogleModal(todo, isEdit) {
  ...
  const btnShareTodo = document.getElementById("shareTodoButton");
  const newBtn = btnShareTodo.cloneNode(true);
  btnShareTodo.parentNode.replaceChild(newBtn, btnShareTodo);
...
Fecha: ${todo.date}
Gestioná tus tareas en  https://todoList.com
`;

    navigator
      .share({
        title: `Tarea: ${todo.title}`,
        text: mensaje,
        url: "https://todoList.com",
      })
      .then(() => console.log("Tarea compartida"))
...
function constructAndShowTodoModal(todo) {
  ...
  shareButton.id = "shareTodoButton";
  shareButton.textContent = "Compartir tarea";

  const oldBtn = document.getElementById("shareTodoButton");
  if (oldBtn) {
    oldBtn.remove();
  }

  document.querySelector(".modal-footer").prepend(shareButton);
      ...
      navigator.serviceWorker.getRegistration().then((reg) => {
        if (reg) {
          reg.showNotification(
            "Tiene una tarea pendiente: " + tareaPendiente.title,
            {
